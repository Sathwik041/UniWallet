...
const SolanaWallet = ({ mnemonic, walletCount, selectednet }) => {
    ...
    return ( mnemonic && walletCount > 0 && 
        <div className="px-4 md:px-20 bg-black rounded-lg shadow-lg m-4">
            <h2 className="text-xl font-bold text-center text-gray-200 mb-2">Generated SOL Wallets</h2>
            {wallets.map((wallet, index) => (
                <div key={index} className="mb-4 p-4 bg-gray-900 rounded-lg flex flex-col gap-2">
                    ...
                    <div className="cursor-pointer mt-2 flex flex-col">
                        <label className="text-red-400 mb-2" >Privatekey :</label>
                        <div className="flex flex-col md:flex-row justify-between gap-2">
                        <code className="break-all" onClick={()=>copyToClipboard(wallet.privateKey)} >{visibleIndex === index ? wallet.privateKey : "•".repeat(64)}</code>
                        <button className="border border-white p-2 rounded-md bg-gray-700 cursor-pointer hover:bg-gray-200 hover:text-black shrink-0" onClick={() => setVisibleIndex(visibleIndex === index ? null : index)}>    
